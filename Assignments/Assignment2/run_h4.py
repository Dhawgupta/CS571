...
filename = 'results_h4_5.pkl'
filename2 = 'results_h4_time5.pkl'
...
for step in step_list:
    temp = run_experiment(start_state_list[step], goal ,h3)
    start_time = time.time()
    results[step] = run_experiment(start_state_list[step], goal, h4)
    end_time = time.time()
    times[step] = end_time - start_time
    print("The time taken (sec) {}".format(times[step]))

    temp2 = results[step]
    print("Optimal Path : {}\nPessimistic Heuristic Path : {}".format(temp.depth[temp.goal_no],temp2.depth[temp2.goal_no] ))
    print("saving")
